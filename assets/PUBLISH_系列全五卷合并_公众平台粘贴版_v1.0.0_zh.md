...
> **已 supersede**：请使用 [`PUBLISH_系列全五卷合并_公众平台粘贴版_v1.0.1_zh.md`](./PUBLISH_系列全五卷合并_公众平台粘贴版_v1.0.1_zh.md)（全书测评 P0/P1、配图索引、用语对齐）。

> **2026-06-02** · 系列合并 **v1.0.0** · 卷一～卷五定稿合并（**不替代** 单卷发表稿）  
...
**一句话**：图谱管 **结构**；任务单 + 签收管 **本轮交付依据**（卷三）；摘要管 **跨轮决策蒸馏**；Skill 管 **怎么问、怎么测**。
...
下面数字来自笔者在 **单一后端示例仓** 做的 **跨轮回顾** 对照实验（2026-05），**不是** 线上用户流量，也 **不等于** 模型 API 的 token 账单。实验用事先声明的 **gold 题集**：六类 **代表性收尾场景**（例如 Harness 文档试点、图谱门禁、流式探针、契约/锚点检查 CI、LLM Wiki 层元实验、Harness 与 Wiki 层闭环主任务单等），**每类四题**，在后端仓内答题。
...
稳态维护预算大约 **10%～15%**（**2～5 人**团队；**≤2 人**目标 **≤10%**），前提是 **只维护试点链与按需补分册**，**非**全仓手绘、**非** KPI；**>20%** 应减项或加强自动化（§25.4、§24）。
...
| 「增量图谱 CI 已成熟，可直接照搬」                  | 现阶段强调 **全量 export/入口清单**；增量能力 **未** 作公众稿承诺     |
