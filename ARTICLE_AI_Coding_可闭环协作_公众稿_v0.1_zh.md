...
> 系列主标题固定为 **「AI 编程可闭环协作」**；本篇为 **卷一**。目录与后续卷见 [系列总目录](ARTICLE_AI_Coding_可闭环协作_公众稿_OUTLINE_v1_zh.md)。

| 项 | 内容 |
| --- | --- |
| **系列主标题** | AI 编程可闭环协作 |
| **卷** | 卷一 |
| **本卷副标题** | 怎样才算「做完」——给 Agent 结构，给团队过程 |
| **版本** | **v0.1.11**（与私仓同步 · 发布定稿） |
| **状态** | `release` — 公众发布基线 |
| **日期** | 2026-05-27 |
| **方向** | **AI Coding**；侧重 **工程协作与质量闭环**（研发治理），非模型训练或 Prompt 技巧大全 |
| **读者** | 已用 Cursor / Copilot / Claude Code / Kimi Code 等的研发负责人、Tech Lead、全栈工程师；**也包括希望一个人也能把需求做完的独立开发者 / 小团队 Tech Lead** |
| **系列目录** | 扩充篇章见 [`ARTICLE_AI_Coding_可闭环协作_公众稿_OUTLINE_v1_zh.md`](./ARTICLE_AI_Coding_可闭环协作_公众稿_OUTLINE_v1_zh.md) |
| **仓库** | [GitHub · ai-coding-closed-loop-articles](https://github.com/Cyning12/ai-coding-closed-loop-articles)（本卷全文 · 系列 OUTLINE 同仓） |
...
| --- | --- |
| — | [摘要](#摘要) |
| 0 | [缘起：个人约束与团队经验](#0-缘起个人约束与团队经验) |
| 1 | [为什么「会写代码」不等于「能交付」](#1-为什么会写代码不等于能交付) |
| 2 | [两条轨：结构（图谱）与过程（协作流程）](#2-两条轨结构图谱与过程协作流程) |
| | 2.1 [技术图谱：让 Agent「先看地图再动手」](#21-技术图谱让-agent先看地图再动手) |
| | 2.2 [协作流程：可签收、可合并](#22-harness协作流程--任务单流程让协作可签收可合并) |
| | 2.3 [叠放：缺一条都会「差点意思」](#23-叠放缺一条都会差点意思) |
| 3 | [一轮交付怎样算「做完」：意图、成果、验收](#3-一轮交付怎样算做完意图成果验收) |
| 4 | [与 TDD、Code Review、DevOps 的关系](#4-与-tddcode-reviewdevops-的关系) |
| 5 | [适合谁、不适合谁](#5-适合谁不适合谁) |
| 6 | [最小起步（给想试的读者）](#6-最小起步给想试的读者) |
| 7 | [结语](#7-结语) |

### 系列后续

后续卷（图谱落地、协作流程细节、案例与 FAQ 等）见 [**系列总目录**](ARTICLE_AI_Coding_可闭环协作_公众稿_OUTLINE_v1_zh.md)。
...
你的理解可以概括成：**主图定入口与边界，子图定这次改动的纵深**；从「要改的那一格」钻进去，比从仓库根目录乱翻更快、也更不容易漏关联。卷二会写子图怎么拆、怎么链回主图。
...
> 掘金等若 Mermaid 异常，可用配图：`images/阶段骨架.png`。
...
**推荐叠放**：同一轮需求里，任务单同时引用 **规格 / 图谱入口 / 测试策略**；终轮审核对照 **审查记录 + CI +（如有）实验结论**。
...
> 掘金等若 Mermaid 异常，可用配图：`images/总结.png`。
...
| 版本 | 日期 | 说明 |
| --- | --- | --- |
| **v0.1** | 2026-05-27 | 首版落盘：公众发布基线；自 draft 冻结 |
| v0.1.1 | 2026-05-27 | 本篇目录 + 系列总目录 OUTLINE |
| v0.1.2 | 2026-05-27 | H1 改为「系列主标题 · 卷一：副标题」 |
| v0.1.3 | 2026-05-27 | §0 缘起：个人预算 / Cursor Auto / 跨模型统一输出 |
| v0.1.4 | 2026-05-27 | §0 增补团队使用经验（Review / CI / 个人可执行最小集） |
| **v0.1.5** | 2026-05-27 | 纳入 DeepSeek 测评：摘要措辞、协作流程同义词、图谱入门门槛、读者定位、经验卡片示例、无 CI 降级、系列目录精简 |
| **v0.1.6** | 2026-05-27 | §0 一句：多工具（Kimi / Claude Code）可复用协作流程；读者行补 Kimi Code |
| **v0.1.7** | 2026-05-27 | 二次测评 P2；§2.1/2.2/3/6/7 增补 Mermaid 与任务单模板；全景图可选 PNG 提示词见 `assets/` |
| **v0.1.8** | 2026-05-27 | §2.1 多模块 API 主链路（用户左→意图路由→回答回包）；§2.2 CI 菱形方案 A；§7 叠放图增子图/任务单/经验反哺 |
| **v0.1.9** | 2026-05-27 | §2.1 增补主图/子图分层表 + RAG 召回改动举例（澄清「子图在哪」） |
