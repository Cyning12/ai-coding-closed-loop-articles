...
> [系列总目录](ARTICLE_AI_Coding_可闭环协作_公众稿_OUTLINE_v1_zh.md) · 上一卷：[卷一：怎样才算「做完」](ARTICLE_AI_Coding_可闭环协作_公众稿_v0.1_zh.md)（卷一 · 已发表）
...
| **系列主标题** | AI 编程可闭环协作 |
| **卷** | 卷二 |
| **本卷副标题** | 技术图谱——让 Agent 先看地图再动手 |
| **版本** | **v0.8.2**（与私仓同步 · 发布定稿） |
| **状态** | `release` — 公众发布基线 |
| **日期** | 2026-05-28 |
| **依赖** | 卷一 2.1 主图/子图读法；卷一 6 最小起步 |
| **读者** | 已用 Agent 改代码、愿意维护活图谱的团队；存量项目见 8.2、8.6 |
| **系列目录** | [`ARTICLE_AI_Coding_可闭环协作_公众稿_OUTLINE_v1_zh.md`](./ARTICLE_AI_Coding_可闭环协作_公众稿_OUTLINE_v1_zh.md) |
...
若你还没读过 **[卷一：怎样才算「做完」](ARTICLE_AI_Coding_可闭环协作_公众稿_v0.1_zh.md)**，建议先看——那里讲清了 **图谱 + 协作流程** 如何叠放，以及「意图 / 成果 / 验收」怎样算一轮交付。
...
**怎么读这张图**：**菱形** = 判断（是/否）；**矩形** = 动作（去读什么、去做什么）；沿箭头从上到下走即可。

```mermaid
flowchart TD
  Q[收到任务] --> R{只做阶段总结或跨模块回顾?}
  R -->|是| W[经验/复盘文档 索引与摘要]
  R -->|否| T{要改接口 / 表 / 契约 / 流程?}
  T -->|否| CODE[读源码 + 任务单即可]
  T -->|是| GQ[GPS：查询子图]
...
| 分支                  | 含义                                                                                          |
| ------------------- | ------------------------------------------------------------------------------------------- |
| **左：阶段总结 / 跨模块回顾**  | 打开 **经验或复盘类文档**（若有）；用于理解背景，**不能**代替改代码时的 GPS+便签。                                           |
| **中：与接口/表/契约/流程无关** | **源码 + 任务单** 往往够用，不必为了「有图谱」而开图谱夹。                                                           |
| **右：改代码链**          | 默认 **GPS（查询子图）→ 站牌/路口便签**；动表或元数据再开 **表结构说明与实现规约**；流程细节仍不够才看 **分册流程图片段**；最后落到 **锚点代码**——图是导航，不是替代实现。 |
